...
class _FakeDatabaseProvider implements DatabaseProvider {
  _FakeDatabaseProvider(this.name, {this.databases = const <DatabaseInfo>[], this.result});
...
void main() {
  ...
  test('getDatabases 合并所有提供者的结果 / getDatabases merges results across providers',
      () async {
    DatabaseRegistry.instance.registerProvider(
      _FakeDatabaseProvider(
        'svc-a',
        databases: [DatabaseInfo(name: 'a', path: '/a', tables: const <TableInfo>[])],
      ),
    );
    DatabaseRegistry.instance.registerProvider(
      _FakeDatabaseProvider(
        'svc-b',
        databases: [DatabaseInfo(name: 'b', path: '/b', tables: const <TableInfo>[])],
      ),
    );
  ...
  test('getDatabases 隔离抛错的提供者 / getDatabases isolates a throwing provider',
      () async {
  ...
  test('queryTable 返回第一个非空结果 / queryTable returns the first non-empty result',
      () async {
    DatabaseRegistry.instance.registerProvider(
      _FakeDatabaseProvider('svc-a', result: QueryResult(rows: const [], columns: const [])),
    );
    DatabaseRegistry.instance.registerProvider(
      _FakeDatabaseProvider(
        'svc-b',
        result: QueryResult(rows: <Map<String, dynamic>>[
          {'id': 1}
        ], columns: ['id']),
      ),
    );
  ...
  test('queryTable 容忍抛错的提供者 / queryTable tolerates a throwing provider',
      () async {
    DatabaseRegistry.instance.registerProvider(_ThrowingDatabaseProvider());
    DatabaseRegistry.instance.registerProvider(
      _FakeDatabaseProvider(
